...
import io.github.stoneream.dachshund.daemon.config.UserNewReleaseNotificationDeliveryJobConfig
...
import io.github.stoneream.dachshund.infra.db.ex.{ArtistReleaseSource, ReleaseTrackSource, UserNewReleaseNotificationQueueSource, UserPlaylistSettingSource, UserSource, UserSpotifyAuthorizationRefreshQueueSource, UserSpotifyAuthorizationSource}
import io.github.stoneream.dachshund.infra.db.generated.{ArtistReleaseDbRow, ReleaseTrackDbRow, UserDbRow, UserNewReleaseNotificationQueueDbRow, UserPlaylistSettingDbRow, UserSpotifyAuthorizationDbRow, UserSpotifyAuthorizationRefreshQueueDbRow}
import io.github.stoneream.dachshund.infra.db.transaction.DatabaseRole
import io.github.stoneream.dachshund.infra.db.writer.{ArtistReleasesWriter, SpotifyAuthorizationRefreshQueueWriter, SpotifyAuthorizationWriter, SpotifyUserWriter, UserNewReleaseNotificationQueueWriter, UserPlaylistSettingWriter}
...
class UserNewReleaseNotificationDeliveryHandlerSpec extends AnyFeatureSpec with DaemonHandlerDatabaseSpecSupport with IdiomaticMockito {
  import UserNewReleaseNotificationDeliveryHandlerSpec.*

  private given LoggingContext = LoggingContext("user-new-release-notification-delivery-handler-spec")
  ...
  Feature("User new release notification delivery job handler") {
  ...
  private def createHandler(
      ...
      config: Option[UserNewReleaseNotificationDeliveryJobConfig] = None
  ): UserNewReleaseNotificationDeliveryHandler = {
    val module = new AbstractModule {
      override def configure(): Unit = {
        ...
        config.foreach(value => bind(classOf[UserNewReleaseNotificationDeliveryJobConfig]).toInstance(value))
      }
    }
    createInjector(fixedNow, module).getInstance(classOf[UserNewReleaseNotificationDeliveryHandler])
  }
  ...
  private def writeNotificationQueue(
    ...
    sql"""
      insert into user_new_release_notification_queue (
...
private object UserNewReleaseNotificationDeliveryHandlerSpec {
