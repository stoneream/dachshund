...
```sql
CREATE TABLE `user_new_release_notification_delivery_queue` (
  ...
  PRIMARY KEY (`id`),
  UNIQUE KEY `id` (`id`),
  UNIQUE KEY `uq_unr_delivery_queue_event_type_playlist` (`user_new_release_event_id`,`release_notification_type`,`playlist_setting_id`),
  KEY `idx_unr_delivery_queue_target` (`deleted`,`status`,`next_attempt_at`,`locked_until`,`id`),
  KEY `idx_unr_delivery_queue_playlist_setting` (`playlist_setting_id`,`deleted`,`status`),
  CONSTRAINT `fk_unr_delivery_queue_event_id` FOREIGN KEY (`user_new_release_event_id`) REFERENCES `user_new_release_event` (`id`),
  CONSTRAINT `fk_unr_delivery_queue_playlist_setting_id` FOREIGN KEY (`playlist_setting_id`) REFERENCES `user_playlist_setting` (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='ユーザー別新着リリース通知配信キュー'
```
...
| ---- | ---- | ---------- |
| fk_unr_delivery_queue_event_id | FOREIGN KEY | FOREIGN KEY (user_new_release_event_id) REFERENCES user_new_release_event (id) |
| fk_unr_delivery_queue_playlist_setting_id | FOREIGN KEY | FOREIGN KEY (playlist_setting_id) REFERENCES user_playlist_setting (id) |
| id | UNIQUE | UNIQUE KEY id (id) |
| PRIMARY | PRIMARY KEY | PRIMARY KEY (id) |
| uq_unr_delivery_queue_event_type_playlist | UNIQUE | UNIQUE KEY uq_unr_delivery_queue_event_type_playlist (user_new_release_event_id, release_notification_type, playlist_setting_id) |

## Indexes

| Name | Definition |
| ---- | ---------- |
| idx_unr_delivery_queue_playlist_setting | KEY idx_unr_delivery_queue_playlist_setting (playlist_setting_id, deleted, status) USING BTREE |
| idx_unr_delivery_queue_target | KEY idx_unr_delivery_queue_target (deleted, status, next_attempt_at, locked_until, id) USING BTREE |
| PRIMARY | PRIMARY KEY (id) USING BTREE |
| id | UNIQUE KEY id (id) USING BTREE |
| uq_unr_delivery_queue_event_type_playlist | UNIQUE KEY uq_unr_delivery_queue_event_type_playlist (user_new_release_event_id, release_notification_type, playlist_setting_id) USING BTREE |
