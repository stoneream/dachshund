...
CREATE TABLE `followed_artist_sync_queue` (
  ...
  `after_cursor` varchar(255) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '次ページ取得用afterカーソル',
  ...
  PRIMARY KEY (`id`),
  UNIQUE KEY `id` (`id`),
  UNIQUE KEY `uq_followed_artist_sync_queue_user_date` (`user_id`,`sync_date`),
  KEY `idx_followed_artist_sync_queue_target` (`deleted`,`status`,`next_attempt_at`,`locked_until`,`id`),
  KEY `idx_followed_artist_sync_queue_user_status` (`user_id`,`status`,`created_at`),
  CONSTRAINT `fk_followed_artist_sync_queue_user_id` FOREIGN KEY (`user_id`) REFERENCES `user` (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='フォロー中アーティスト同期キュー'
```
...
| sync_date | date |  | false |  |  |  | 同期対象日 |
| status | varchar(255) | SCHEDULED | false |  |  |  | キュー状態(SCHEDULED, PROCESSING, SUCCEEDED, FAILED, BLOCKED, SKIPPED) |
| requested_limit | int |  | false |  |  |  | Spotify API取得件数 |
| after_cursor | varchar(255) |  | true |  |  |  | 次ページ取得用afterカーソル |
...
| ---- | ---- | ---------- |
| fk_followed_artist_sync_queue_user_id | FOREIGN KEY | FOREIGN KEY (user_id) REFERENCES user (id) |
| id | UNIQUE | UNIQUE KEY id (id) |
| PRIMARY | PRIMARY KEY | PRIMARY KEY (id) |
| uq_followed_artist_sync_queue_user_date | UNIQUE | UNIQUE KEY uq_followed_artist_sync_queue_user_date (user_id, sync_date) |

## Indexes

| Name | Definition |
| ---- | ---------- |
| idx_followed_artist_sync_queue_target | KEY idx_followed_artist_sync_queue_target (deleted, status, next_attempt_at, locked_until, id) USING BTREE |
| idx_followed_artist_sync_queue_user_status | KEY idx_followed_artist_sync_queue_user_status (user_id, status, created_at) USING BTREE |
| PRIMARY | PRIMARY KEY (id) USING BTREE |
| id | UNIQUE KEY id (id) USING BTREE |
| uq_followed_artist_sync_queue_user_date | UNIQUE KEY uq_followed_artist_sync_queue_user_date (user_id, sync_date) USING BTREE |
