...
CREATE TABLE `user_spotify_auth` (
  ...
  `spotify_user_id` varchar(255) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT 'SpotifyユーザーID',
  ...
  PRIMARY KEY (`id`),
  UNIQUE KEY `id` (`id`),
  UNIQUE KEY `uq_user_spotify_auth_user_id` (`user_id`),
  UNIQUE KEY `uq_user_spotify_auth_spotify_user_id` (`spotify_user_id`),
  CONSTRAINT `fk_user_spotify_auth_user_id` FOREIGN KEY (`user_id`) REFERENCES `user` (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=[Redacted by tbls] DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Spotify認証識別子'
```
...
| spotify_user_id | varchar(255) |  | false |  |  |  | SpotifyユーザーID |
...
| ---- | ---- | ---------- |
| fk_user_spotify_auth_user_id | FOREIGN KEY | FOREIGN KEY (user_id) REFERENCES user (id) |
| id | UNIQUE | UNIQUE KEY id (id) |
| PRIMARY | PRIMARY KEY | PRIMARY KEY (id) |
| uq_user_spotify_auth_spotify_user_id | UNIQUE | UNIQUE KEY uq_user_spotify_auth_spotify_user_id (spotify_user_id) |
| uq_user_spotify_auth_user_id | UNIQUE | UNIQUE KEY uq_user_spotify_auth_user_id (user_id) |
...
| uq_user_spotify_auth_spotify_user_id | UNIQUE KEY uq_user_spotify_auth_spotify_user_id (spotify_user_id) USING BTREE |
| uq_user_spotify_auth_user_id | UNIQUE KEY uq_user_spotify_auth_user_id (user_id) USING BTREE |
