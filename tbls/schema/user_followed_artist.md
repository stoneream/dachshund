...
CREATE TABLE `user_followed_artist` (
  ...
  `artist_name` varchar(512) COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT '' COMMENT 'アーティスト名',
  `spotify_artist_uri` varchar(255) COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT '' COMMENT 'SpotifyアーティストURI',
  ...
  `followers_total` bigint DEFAULT NULL COMMENT 'Spotifyフォロワー数',
  `popularity` int DEFAULT NULL COMMENT 'Spotify人気度',
  `first_followed_at` datetime DEFAULT NULL COMMENT '初回フォロー検出日時',
  `last_seen_at` datetime DEFAULT NULL COMMENT '最終検出日時',
  ...
  PRIMARY KEY (`id`),
  UNIQUE KEY `id` (`id`),
  UNIQUE KEY `uq_user_followed_artist_user_artist` (`user_id`,`spotify_artist_code`),
  KEY `idx_user_followed_artist_sync_target` (`deleted`,`user_id`,`last_synced_at`),
  KEY `idx_user_followed_artist_artist_code` (`spotify_artist_code`),
  CONSTRAINT `fk_user_followed_artist_user_id` FOREIGN KEY (`user_id`) REFERENCES `user` (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='ユーザー別フォロー中アーティスト'
```
...
| artist_name | varchar(512) |  | false |  |  |  | アーティスト名 |
| spotify_artist_uri | varchar(255) |  | false |  |  |  | SpotifyアーティストURI |
...
| followers_total | bigint |  | true |  |  |  | Spotifyフォロワー数 |
| popularity | int |  | true |  |  |  | Spotify人気度 |
| first_followed_at | datetime |  | true |  |  |  | 初回フォロー検出日時 |
| last_seen_at | datetime |  | true |  |  |  | 最終検出日時 |
| last_synced_at | datetime |  | true |  |  |  | 最終同期日時 |
...
| ---- | ---- | ---------- |
| fk_user_followed_artist_user_id | FOREIGN KEY | FOREIGN KEY (user_id) REFERENCES user (id) |
| id | UNIQUE | UNIQUE KEY id (id) |
| PRIMARY | PRIMARY KEY | PRIMARY KEY (id) |
| uq_user_followed_artist_user_artist | UNIQUE | UNIQUE KEY uq_user_followed_artist_user_artist (user_id, spotify_artist_code) |

## Indexes

| Name | Definition |
| ---- | ---------- |
| idx_user_followed_artist_artist_code | KEY idx_user_followed_artist_artist_code (spotify_artist_code) USING BTREE |
| idx_user_followed_artist_sync_target | KEY idx_user_followed_artist_sync_target (deleted, user_id, last_synced_at) USING BTREE |
| PRIMARY | PRIMARY KEY (id) USING BTREE |
| id | UNIQUE KEY id (id) USING BTREE |
| uq_user_followed_artist_user_artist | UNIQUE KEY uq_user_followed_artist_user_artist (user_id, spotify_artist_code) USING BTREE |
