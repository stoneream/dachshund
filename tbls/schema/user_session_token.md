...
CREATE TABLE `user_session_token` (
  ...
  `hashed_token` varchar(255) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT 'セッショントークンのハッシュ値',
  ...
  PRIMARY KEY (`id`),
  UNIQUE KEY `id` (`id`),
  UNIQUE KEY `uq_user_session_token_hashed_token` (`hashed_token`),
  KEY `idx_user_session_token_user_id` (`user_id`),
...
| hashed_token | varchar(255) |  | false |  |  |  | セッショントークンのハッシュ値 |
| issued_at | datetime |  | false |  |  |  | トークン発行日時 |
| last_accessed_at | datetime |  | false |  |  |  | 最終アクセス日時 |
| idle_expires_at | datetime |  | false |  |  |  | アイドルタイムアウト日時 |
| expires_at | datetime |  | false |  |  |  | トークン有効期限日時 |
...
| ---- | ---- | ---------- |
| fk_user_session_token_user_id | FOREIGN KEY | FOREIGN KEY (user_id) REFERENCES user (id) |
| id | UNIQUE | UNIQUE KEY id (id) |
| PRIMARY | PRIMARY KEY | PRIMARY KEY (id) |
| uq_user_session_token_hashed_token | UNIQUE | UNIQUE KEY uq_user_session_token_hashed_token (hashed_token) |

## Indexes

| Name | Definition |
| ---- | ---------- |
| idx_user_session_token_expires_at | KEY idx_user_session_token_expires_at (expires_at) USING BTREE |
| idx_user_session_token_idle_expires_at | KEY idx_user_session_token_idle_expires_at (idle_expires_at) USING BTREE |
| idx_user_session_token_user_id | KEY idx_user_session_token_user_id (user_id) USING BTREE |
| PRIMARY | PRIMARY KEY (id) USING BTREE |
| id | UNIQUE KEY id (id) USING BTREE |
| uq_user_session_token_hashed_token | UNIQUE KEY uq_user_session_token_hashed_token (hashed_token) USING BTREE |
