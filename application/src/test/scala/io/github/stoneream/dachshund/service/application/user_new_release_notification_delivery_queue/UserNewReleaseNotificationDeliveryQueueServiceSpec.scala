...
import io.github.stoneream.dachshund.infra.db.ex.{ArtistReleaseSource, UserNewReleaseNotificationDeliveryQueueSource, UserPlaylistSettingSource, UserSource}
import io.github.stoneream.dachshund.infra.db.generated.{ArtistReleaseDbRow, UserDbRow, UserNewReleaseNotificationDeliveryQueueDbRow, UserPlaylistSettingDbRow}
...
import io.github.stoneream.dachshund.infra.db.writer.{ArtistReleasesWriter, SpotifyUserWriter, UserNewReleaseNotificationDeliveryQueueWriter, UserPlaylistSettingWriter}
import io.github.stoneream.dachshund.lib.datetime.BusinessDateTime
import io.github.stoneream.dachshund.model.{PlaylistUsageType, QueueJobStatus, ReleaseNotificationType}
import io.github.stoneream.dachshund.service.application.user_new_release_notification_delivery_queue.UserNewReleaseNotificationDeliveryQueueUpdateResult.Updated
...
class UserNewReleaseNotificationDeliveryQueueServiceSpec extends AnyFeatureSpec with ScalaFutures with DatabaseSupport {
  ...
  private val service = new UserNewReleaseNotificationDeliveryQueueServiceImpl(
    databaseTransaction = databaseTransaction,
    queueReader = new UserNewReleaseNotificationDeliveryQueueReader,
  ...
  private def findQueueTarget(userNewReleaseEventId: Long): UserNewReleaseNotificationDeliveryQueueTarget =
    databaseTransaction.readOnly(DatabaseRole.Master) { implicit session =>
      sql"""
        select
          ...
          q.lock_version
        from
          user_new_release_notification_delivery_queue q
  ...
  private object Rows {
    ...
    def scheduledQueueRow(userNewReleaseEventId: Long, playlistSettingId: Long): UserNewReleaseNotificationDeliveryQueueDbRow =
      queueRow(
    ...
    def futureQueueRow(userNewReleaseEventId: Long, playlistSettingId: Long): UserNewReleaseNotificationDeliveryQueueDbRow =
      scheduledQueueRow(userNewReleaseEventId, playlistSettingId).copy(nextAttemptAt = Some(fixedNow.plus(1.hour).toLocalDateTime))

    def staleQueueRow(userNewReleaseEventId: Long, playlistSettingId: Long): UserNewReleaseNotificationDeliveryQueueDbRow =
      queueRow(
        userNewReleaseEventId = userNewReleaseEventId,
        playlistSettingId = playlistSettingId,
