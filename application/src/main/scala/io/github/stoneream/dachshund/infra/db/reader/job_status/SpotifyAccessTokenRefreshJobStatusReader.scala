...
object SpotifyAccessTokenRefreshJobStatusReader {
...
@Singleton
class SpotifyAccessTokenRefreshJobStatusReader @Inject() () {
  import SpotifyAccessTokenRefreshJobStatusReader.{QueueRow, StatusCount}
  ...
  def countByStatus()(using DBSession): Seq[StatusCount] =
    sql"""
      ...
      from
        user_spotify_authorization_refresh_queue q
        inner join user_spotify_authorization usa
          on usa.id = q.authorization_id
          and usa.deleted = 0
        inner join user u
          on u.id = usa.user_id
          and u.deleted = 0
  ...
  def findQueueRows(
    ...
    """
      ...
      .map { row =>
        queueRow(
          row,
          targetLabel = s"authorization_id=${row.long("authorization_id")}"
        )
      }
      .list
      .apply()
