...
import io.github.stoneream.dachshund.infra.db.reader.job_status.SpotifyAccessTokenRefreshJobStatusReader
import io.github.stoneream.dachshund.infra.db.reader.job_status.SpotifyAccessTokenRefreshJobStatusReader.QueueRow as ReaderQueueRow
...
@Singleton
class SpotifyAccessTokenRefreshJobStatusUseCase @Inject() (
    databaseTransaction: DatabaseTransaction,
    reader: SpotifyAccessTokenRefreshJobStatusReader,
  ...
  override def run(input: JobStatusDetailUseCaseInput)(using LoggingContext): Future[SpotifyAccessTokenRefreshJobStatusUseCaseOutput] =
    show(input)

  private def show(input: JobStatusDetailUseCaseInput): Future[SpotifyAccessTokenRefreshJobStatusUseCaseOutput] =
    readStatus(input).map { case (counts, rows, detailPage) =>
      SpotifyAccessTokenRefreshJobStatusUseCaseOutput(
        context = JobStatusDetailPageContext.build(
          ...
          currentJob = JobStatusJob.SpotifyAccessTokenRefresh,
