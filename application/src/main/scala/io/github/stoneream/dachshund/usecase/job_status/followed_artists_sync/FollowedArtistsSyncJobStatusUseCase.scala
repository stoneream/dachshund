...
import io.github.stoneream.dachshund.infra.db.reader.job_status.FollowedArtistsSyncJobStatusReader
import io.github.stoneream.dachshund.infra.db.reader.job_status.FollowedArtistsSyncJobStatusReader.QueueRow as ReaderQueueRow
...
@Singleton
class FollowedArtistsSyncJobStatusUseCase @Inject() (
    databaseTransaction: DatabaseTransaction,
    reader: FollowedArtistsSyncJobStatusReader,
  ...
  override def run(input: JobStatusDetailUseCaseInput)(using LoggingContext): Future[FollowedArtistsSyncJobStatusUseCaseOutput] =
    show(input)

  private def show(input: JobStatusDetailUseCaseInput): Future[FollowedArtistsSyncJobStatusUseCaseOutput] =
    readStatus(input).map { case (counts, rows, detailPage) =>
      FollowedArtistsSyncJobStatusUseCaseOutput(
        context = JobStatusDetailPageContext.build(
          ...
          currentJob = JobStatusJob.FollowedArtistsSync,
