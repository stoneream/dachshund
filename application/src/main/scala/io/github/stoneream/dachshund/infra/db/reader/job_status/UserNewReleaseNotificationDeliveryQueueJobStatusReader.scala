...
object UserNewReleaseNotificationDeliveryQueueJobStatusReader {
...
@Singleton
class UserNewReleaseNotificationDeliveryQueueJobStatusReader @Inject() () {
  import UserNewReleaseNotificationDeliveryQueueJobStatusReader.{QueueRow, StatusCount}
  ...
  def findQueueRows(
    ...
    """
      ...
      .map { row =>
        queueRow(
          row,
          targetLabel = s"user_new_release_event_id=${row.long("user_new_release_event_id")}, " +
            s"release_notification_type=${row.string("release_notification_type")}, " +
            s"playlist_setting_id=${row.long("playlist_setting_id")}"
        )
      }
      .list
      .apply()
