...
import io.github.stoneream.dachshund.infra.db.reader.job_status.UserNewReleaseNotificationDeliveryJobStatusReader
import io.github.stoneream.dachshund.infra.db.reader.job_status.UserNewReleaseNotificationDeliveryJobStatusReader.QueueRow as ReaderQueueRow
...
@Singleton
class UserNewReleaseNotificationDeliveryJobStatusUseCase @Inject() (
    databaseTransaction: DatabaseTransaction,
    reader: UserNewReleaseNotificationDeliveryJobStatusReader,
  ...
  override def run(input: JobStatusDetailUseCaseInput)(using LoggingContext): Future[UserNewReleaseNotificationDeliveryJobStatusUseCaseOutput] =
    show(input)

  private def show(input: JobStatusDetailUseCaseInput): Future[UserNewReleaseNotificationDeliveryJobStatusUseCaseOutput] =
    readStatus(input).map { case (counts, rows, detailPage) =>
      UserNewReleaseNotificationDeliveryJobStatusUseCaseOutput(
        context = JobStatusDetailPageContext.build(
          ...
          currentJob = JobStatusJob.UserNewReleaseNotificationDelivery,
