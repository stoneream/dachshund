...
@Singleton
class UserNewReleaseNotificationQueueWriter @Inject() () {
  def write(row: UserNewReleaseNotificationQueueDbRow)(using DBSession): Int =
    sql"""
      insert ignore into user_new_release_notification_queue (
