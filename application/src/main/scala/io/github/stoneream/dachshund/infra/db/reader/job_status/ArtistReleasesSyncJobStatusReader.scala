...
object ArtistReleasesSyncJobStatusReader {
...
@Singleton
class ArtistReleasesSyncJobStatusReader @Inject() () {
  import ArtistReleasesSyncJobStatusReader.{QueueRow, StatusCount}
      ...
      from
        ...
        inner join (
          ...
          where
            ...
            and u.deleted = 0
            and u.enabled = 1
        ) target on target.spotify_artist_code = q.spotify_artist_code
      where
        q.deleted = 0
      group by
        q.status
  ...
  def findQueueRows(
    ...
    """
      ...
      .map { row =>
        queueRow(
          row,
          targetLabel = s"spotify_artist_code=${row.string("spotify_artist_code")}, sync_scope=${row.string("sync_scope")}"
        )
      }
      .list
      .apply()
